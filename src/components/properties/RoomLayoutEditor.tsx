...
export function RoomLayoutEditor({ value, onChange }: {
  ...
  return <div style={{ display: "grid", gap: 12 }}>
    <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
      <div>
        <div style={{ fontWeight: 900 }}>Room & bed layout</div>
        <div style={{ fontSize: 12, color: "#6b7280", marginTop: 3 }}>
          Keep bedrooms separate from other areas that provide a bed.
        </div>
      </div>
      <div style={{ display: "flex", gap: 8 }}>
        <button type="button" onClick={() => add("BEDROOM")}>+ Bedroom</button>
        <button type="button" onClick={() => add("SLEEPING_AREA")}>+ Other area</button>
      </div>
    </div>
    ...
    {value.map((area, ai) => <div key={ai} style={{
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 8 }}>
        <select style={control} value={area.kind} onChange={e => patch(ai, { kind: e.target.value as SleepingArea["kind"] })}>
          ...
          <option value="SLEEPING_AREA">Other area</option>
        </select>
      ...
      {area.beds.map((bed, bi) => <div key={bi} style={{
        ...
        <button type="button" onClick={() => patch(ai, { beds: area.beds.filter((_, i) => i !== bi) })}>Remove</button>
      </div>)}

      <div style={{ display: "flex", gap: 8 }}>
        <button type="button" onClick={() => patch(ai, { beds: [...area.beds, { type: "QUEEN", quantity: 1 }] })}>+ Bed</button>
        <button type="button" onClick={() => onChange(value.filter((_, i) => i !== ai).map((item, i) => ({ ...item, sortOrder: i })))}>Remove area</button>
      </div>
    </div>)}
  </div>;
}
