  ...
  async function runFullSync() {
    ...
    try {
      ...
      if (!response.ok || payload.ok !== true) {
        const retryCopy = payload.retryAt
          ? ` Retry available after ${new Date(payload.retryAt).toLocaleString()}.`
          : "";
        throw new Error(
          `${payload.error || "Unable to request Full Sync."}${retryCopy}`
        );
      }

      const kinds = payload.result?.messageKinds?.join(" + ") ||
        "AVAILABILITY + RATES_RESTRICTIONS";
      setMessage(`Full Sync queued successfully: ${kinds}.`);
    } catch (caught) {
      setError(
        caught instanceof Error
          ? caught.message
          : "Unable to request Full Sync."
      );
    } finally {
      setRunning(false);
    }
      ...
      >
        <div>
          ...
          <div style={{ marginTop: 4, fontSize: 13, color: "#6b7280" }}>
            Send a complete availability and rates/restrictions sync for this property.
          </div>
        </div>
