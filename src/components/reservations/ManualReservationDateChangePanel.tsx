...
export function ManualReservationDateChangePanel() {
    ...
    async function load() {
      ...
      try {
        const response = await fetch(
          `${API_BASE}/api/dashboard/reservations/${encodeURIComponent(id)}`,
          { credentials: "include" }
        );
  ...
  if (
    !reservation ||
    String(reservation.source ?? "").toUpperCase() !== "MANUAL" ||
  ...
  async function save() {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE}/api/dashboard/reservations/${encodeURIComponent(id)}/dates`,
        {
          method: "PATCH",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            checkIn: `${checkIn}T16:00:00-04:00`,
            checkOut: `${checkOut}T11:00:00-04:00`,
          }),
        }
      );
      ...
      if (!response.ok || payload?.ok !== true) {
        throw new Error(
          payload?.message || payload?.error || "Unable to update reservation dates."
        );
      }
      ...
      setError(
        caught instanceof Error
          ? caught.message
          : "Unable to update reservation dates."
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    ...
    >
      ...
      >
        <div>
          ...
          <div style={{ marginTop: 4, fontSize: 13, color: "#6b7280" }}>
            Change the dates for this manual reservation.
          </div>
        </div>
      ...
      {error ? (
        <div style={{ marginTop: 12, color: "#991b1b", fontSize: 13, fontWeight: 600 }}>
          {error}
        </div>
      ) : null}
      {message ? (
