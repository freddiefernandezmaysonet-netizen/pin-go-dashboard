...
test("connection return displays only verified resource ownership without later-phase controls", async () => {
  ...
  assert.equal(p.states[0], "CHANNEL_READ"); assert.match(text(node), /Retorno verificado/); assert.match(text(node), /esta verificación no modifica la sincronización existente/);
  assert.doesNotMatch(text(node), /Autorización confirmada|Airbnb confirmó|test-only-state|Consultar anuncios|propuesta de mapeo|Falta confirmar la cuenta/);
